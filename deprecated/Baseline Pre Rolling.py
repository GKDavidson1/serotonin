...
def seed_based_fc(bold, seed_idx):
    ...
    for a in range(bold.shape[1]):
        ...
        if not np.isfinite(area_std) or np.isclose(area_std, 0.0):
            continue
        corr = np.corrcoef(seed, area_ts)[0, 1]
        if np.isfinite(corr):
            fc[a] = float(corr)
...
FITNESS_CONFIG = {
    ...
    'fc_drive_gain': 1.0,
    'fc_baseline_start_seconds': 1.0,
    'fc_baseline_end_seconds': 7.0,
    'fc_balloon_gain': 0.075,
...
def _resolve_model_seed_index(area_names, *, seed_area=None, seed_idx=None):
    if seed_area is not None:
        if seed_area not in area_names:
            raise ValueError(f"seed_area '{seed_area}' not found in model areas.")
        return int(area_names.index(seed_area))

    if seed_idx is None:
        raise ValueError("Either seed_area or seed_idx must be provided for FC fitness.")
...
def compute_seed_fc_from_currents(I_total_abs, params_run, area_names, *, fitness_config):
    config = FITNESS_CONFIG.copy()
    config.update(fitness_config or {})

    seed_idx = _resolve_model_seed_index(
        area_names,
    ...
    if trim_steps >= I_total_abs.shape[0] - 1:
        raise ValueError("fc_trim_seconds removes the full simulation.")

    drive = itotal_to_drive_abs(
        I_total_abs[trim_steps:, :, :],
        unit=brian2.pA,
        gain=float(config['fc_drive_gain']),
    )
    ...
    fc_vector = seed_based_fc(bold, seed_idx)

    return {
        'seed_idx': seed_idx,
        'seed_area': area_names[seed_idx],
        'seed_fc': fc_vector,
...
def summarise_simulation(simulation, fitness_config=None):
    ...
    if target_csv:
        ...
        model_fc = compute_seed_fc_from_currents(
            simulation['I_total_abs'],
            simulation['params'],
            simulation['area_names'],
            fitness_config=config,
        )
        valid_mask = np.isfinite(model_fc['seed_fc']) & np.isfinite(target_fc)
        if not np.any(valid_mask):
            return None

        summary.update({
            'target_fc': target_fc,
            'seed_fc': model_fc['seed_fc'],
...
def fitness_from_summary(summary, fitness_config=None):
    ...
    if 'seed_fc' in summary and 'target_fc' in summary:
        valid_mask = np.isfinite(summary['seed_fc']) & np.isfinite(summary['target_fc'])
        if not np.any(valid_mask):
            return FAILURE_PENALTY
...
def save_checkpoint(checkpoint_path, *, es, generation, log_dir, fitness_config):
    ...
    payload = {
        ...
        'parameter_names': [name for (name, _, _, _, _, _) in PARAMETER_SPACE],
    }
...
def load_checkpoint(checkpoint_path):
    ...
    expected_parameter_names = [name for (name, _, _, _, _, _) in PARAMETER_SPACE]
    saved_parameter_names = payload.get('parameter_names')
    if saved_parameter_names != expected_parameter_names:
        raise ValueError(
            "Checkpoint parameter space does not match the current code. "
            "Refuse to resume with inconsistent optimiser dimensions."
        )
...
def export_best_fc_csvs(log_dir, best_vector, fitness_config=None):
    simulation = run_simulation(best_vector)
    summary = summarise_simulation(simulation, fitness_config=fitness_config)
    ...
    seed_idx = int(summary['seed_idx'])
    model_fc = np.asarray(summary['seed_fc'], dtype=float)
    target_fc = np.asarray(summary['target_fc'], dtype=float)
    valid_mask = np.isfinite(model_fc) & np.isfinite(target_fc)
    diff = model_fc - target_fc
    squared_error = diff ** 2

    common_columns = {
        'area_name': area_names,
        'seed_area': [seed_area] * len(area_names),
        'seed_idx': [seed_idx] * len(area_names),
    }
    ...
    pandas.DataFrame({
        **common_columns,
        'model_seed_fc': model_fc,
    }).to_csv(seed_fc_path, index=False)

    pandas.DataFrame({
        **common_columns,
        'target_seed_fc': target_fc,
...
def run_cmaes(
    *,
    ...
    if resume_path is not None:
        ...
        log_dir = Path(checkpoint.get('log_dir', log_dir))
    else:
        checkpoint = None
    ...
    checkpoint_path = log_dir / 'latest_checkpoint.pkl'
    ...
    if checkpoint is None:
        ...
        generation = 0
        effective_fitness_config = dict(fitness_config or {})
        save_checkpoint(
            checkpoint_path,
            es=es,
        ...
        effective_fitness_config = dict(checkpoint.get('fitness_config') or {})
        if fitness_config:
            effective_fitness_config.update(fitness_config)

        if sigma0 is not None:
            raise ValueError("Cannot change sigma0 when resuming from a checkpoint.")
        if popsize is not None:
            raise ValueError("Cannot change popsize when resuming from a checkpoint.")
        if seed is not None:
            raise ValueError("Cannot change seed when resuming from a checkpoint.")

        if maxfevals is not None:
            es.opts['maxfevals'] = int(maxfevals)
        if ftarget is not None:
            es.opts['ftarget'] = float(ftarget)
    ...
    while not es.stop():
        population = es.ask()
        fitness_values = parallel_fitness(
            population,
            workers=workers,
            fitness_config=effective_fitness_config,
        )
    ...
    exported_files = {}
    try:
        exported = export_best_fc_csvs(log_dir, best_vector, fitness_config=effective_fitness_config)
...
def parse_args():
    ...
    #   Resume from the latest checkpoint written during an earlier run:
    #     py -3 "Baseline 2.py" --resume-from "cmaes_logs/latest_checkpoint.pkl"
    # Notes:
    #   When resuming, do not pass --sigma0, --popsize, or --seed; the checkpoint already
    #   contains the live CMA-ES state. You may still extend --maxfevals or set --ftarget.
...
if __name__ == '__main__':
    # Entry point for terminal usage. Fresh runs omit --resume-from; resumed runs point it
    # at a checkpoint file such as cmaes_logs/latest_checkpoint.pkl.
    main()
